...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    private int _temp_currentPage = 0;
    ...
    public void LerpToPage(int aPageIndex) {
        ...
        if (text_page != null)
        {
            _temp_currentPage = _currentPage + 1;
            text_page.text = _temp_currentPage + "/" + _pageCount;
        }
    }
    ...
    public void EnablePopup(bool _enable, bool _drag)
    {
        GetComponentInParent<Image>().enabled = _enable;
        ...
        _scrollRectComponent.horizontal = _drag;
        nextButton.GetComponent<Button>().interactable = _drag;
        prevButton.GetComponent<Button>().interactable = _drag;
        if(btn_close != null)
            btn_close.gameObject.SetActive(_drag);
    }
    ...
    public void GoToScreen(int _page)
    {
        LerpToPage(_page);
    }
